document.addEventListener('DOMContentLoaded', function() {
    ...
    function filtrarPeliculas(categoria) {
        peliculas.forEach(pelicula => {
            const categoriaPelicula = pelicula.getAttribute('data-categoria');
            const mostrar = categoria === 'todas' || categoriaPelicula === categoria;
            
            pelicula.style.display = mostrar ? 'block' : 'none';
        });
    }

    // 3. Event listeners (solo para 2 categorías de ejemplo)
